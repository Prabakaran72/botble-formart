...
namespace Botble\Testimonial\Exports;
...
class TestimonialExport extends TableExportHandler
{
    protected function afterSheet(AfterSheet $event)
    {
        ...
        for ($index = 2; $index <= $totalRows; $index++) {
            ...
            if ($value == TestimonialStatusEnum::READ) {
            ...
            $event->sheet
                ->getDelegate()
                ->getCell('F' . $index)
                ->setValue(TestimonialStatusEnum::getLabel($value));
        }
    }
}
