@if (!$isNotDefaultLanguage)
    <script id="product_attribute_template" type="text/x-custom-template">
        <li data-id="__id__" class="clearfix">
            <div class="swatch-is-dependent">
                <select name="related_attribute_is_dependent">
                    <option>This is a Child</option>
                    <option>This is a Parent</option>
                </select>
            </div>
            <div class="swatch-dependentcy">
                <select name="related_attribute_is_dependent">
                    <option>This is a Child</option>
                    <option>This is a Parent</option>
                </select>
...
<div class="swatches-container">
    <div class="header clearfix">
        @if (!$isNotDefaultLanguage)
            <div class="swatch-is-dependent">
                {{ trans('plugins/ecommerce::product-attribute-sets.is_dependent') }}
            </div>
        @endif
        @if (!$isNotDefaultLanguage)
        <div class="swatch-dependecy">
            {{ trans('plugins/ecommerce::product-attribute-sets.parent') }}
        </div>
        @endif
        @if (!$isNotDefaultLanguage)
        <div class="swatch-is-default">
        ...
        @if (count($attributes) > 0)
            @foreach($attributes as $attribute)
                <li data-id="{{ $attribute->id }}" class="clearfix">
                    @if (!$isNotDefaultLanguage)
                    <div class="swatch-is-default">
                        <input type="radio" name="related_attribute_is_dependent" value="{{ $attribute->order }}" @if ($attribute->is_default) checked @endif>
                    </div>
                @endif
                    @if (!$isNotDefaultLanguage)
                    <div class="swatch-dependency">
                        <select name="related_attribute_is_dependent">
                            <option value="{{ $attribute->color }}"> This is a Child</option>
                            <option value="{{ $attribute->color }}">This is a Parent</option>
                        </select>
                    </div>
                    @endif
